...
def _split_pasted_line(line: str) -> List[str]:
    """
    Zerlegt eine eingefügte Zeile bevorzugt tab-separiert.
    Fallback auf 2+ Leerzeichen, falls Tabs fehlen.
    """
    if "\t" in line:
...
def parse_pflegeplan_text(raw_text: str) -> List[Dict[str, Any]]:
    ...
    Erwartung:
    - idealerweise tabellarischer Copy/Paste mit Spalten
    - Tabs zwischen den Spalten sind am zuverlässigsten
    - Einrückung kann je nach Quelle mitkopiert werden, muss aber nicht
    """
    ...
    for line in lines:
        ...
        if plain.startswith("Pfleg OE:") or "Fachabt.:" in plain:
            continue
        ...
        if len(parts) == 1:
            ...
            zyklentext = parts[3] if len(parts) > 3 else ""

        row_counter += 1
        indent_hint = get_leading_indent_hint(title_raw)
        ...
        normalized_rows.append(
            {
                ...
                "title_raw": title_raw,
                "status": status,
                "evaluierung": parts[2] if len(parts) > 2 else "",
