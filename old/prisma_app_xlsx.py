"""
PRISMA - XLSX-only Streamlit App
================================

Stabile PRISMA-Oberfläche für den XLSX-Import.

Ziele:
- nur XLSX-Upload
- Anzeige aktiver Einträge
- Status "Abgesetzt" konsequent unsichtbar
- PMD vs. manuell
- Zyklentext sichtbar
- Details standardmäßig ausblenden
"""
...
from prisma_xlsx import parse_pflegeplan_xlsx
...
def entries_to_dataframe(entries: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        # Sicherheitsnetz: selbst wenn etwas durchrutscht, bleibt "Abgesetzt" unsichtbar
...
- PMD vs. manuell
- Zyklentext
- Paket / Leistung / Detail
- Details standardmäßig ausgeblendet
"""
)

with st.expander("Hinweis zur aktuellen Strategie", expanded=False):
    st.markdown(
        """
Der Textimport ist vorerst bewusst pausiert.

Grund:
- XLSX liefert die stabilere Struktur
- so können wir die Fachlogik zuerst sauber aufbauen
- später kann der Textimport separat wieder dazukommen
"""
    )
...
with st.expander("Parser-Statistik", expanded=False):
    stats_df = pd.DataFrame(
        [{"Kennzahl": key, "Wert": value} for key, value in parser_stats.items()]
    )
...
st.divider()
st.success("PRISMA XLSX-only geladen. Nächster Schritt: LEP-/Regelabgleich.")
