...
Verbesserungen:
- vollständige Models/Rules-Module
- Unmatched-Sicht inkl. Vorschlägen
- Paket- und Regelzusammenfassungen
"""
...
from .matcher_v09_1 import apply_matching
from .models_v09_1 import ParsedEntry, RuleFinding
from .rules_v09_1 import MESSAGES_V09_1, RULES_V09_1
...
def check_duplicate_same_service(entries: List[ParsedEntry], counter: Dict[str, int]) -> List[RuleFinding]:
    ...
    if not RULES_V09_1["duplicate_same_service"]["enabled"]:
        return findings
        ...
        for other in group[1:]:
            findings.append(
                RuleFinding(
                    ...
                    severity=RULES_V09_1["duplicate_same_service"]["severity"],
                    ...
                    message=MESSAGES_V09_1["duplicate_same_service"].format(title=anchor.title),
                    confidence=0.95,
                    payload={
...
def check_included_redundancy(entries: List[ParsedEntry], lep_rule_data: Dict[str, Any], counter: Dict[str, int]) -> List[RuleFinding]:
    findings: List[RuleFinding] = []
    if not RULES_V09_1["included_redundancy"]["enabled"]:
        return findings
                ...
                for child in by_lep_id.get(included_id, []):
                    ...
                    findings.append(
                        RuleFinding(
                            ...
                            severity=RULES_V09_1["included_redundancy"]["severity"],
                            ...
                            message=MESSAGES_V09_1["included_redundancy"].format(child=child.title, parent=entry.title),
                            confidence=0.9,
...
def check_excluded_but_separate_check(entries: List[ParsedEntry], lep_rule_data: Dict[str, Any], counter: Dict[str, int]) -> List[RuleFinding]:
    findings: List[RuleFinding] = []
    if not RULES_V09_1["excluded_but_separate_check"]["enabled"]:
        return findings
            ...
            for excluded in lep_rule.get("excluded", []):
                ...
                findings.append(
                    RuleFinding(
                        ...
                        severity=RULES_V09_1["excluded_but_separate_check"]["severity"],
                        package=entry.package,
                        entry_id_primary=entry.entry_id,
                        entry_id_secondary=None,
                        title_primary=entry.display_title,
                        title_secondary=None,
                        message=MESSAGES_V09_1["excluded_but_separate_check"].format(excluded=excluded_title, parent=entry.title),
                        confidence=0.65,
                        payload={
...
def run_engine_v09_1(parser_entries: List[Dict[str, Any]], lep_rules_path: str | Path, lep_aliases_path: str | Path) -> Dict[str, Any]:
