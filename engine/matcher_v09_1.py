"""
PRISMA - Matcher v09.1
======================

Verbesserungen in v09.1:
- Alias-Synonyme
- paketbezogene Alias-Sichtbarkeit
- Vorschläge für unmatched Leistungen
"""
...
from .models_v09_1 import MatchResult, MatchSuggestion, ParsedEntry
...
def build_lookup_tables(alias_data: Dict[str, Any], lep_rule_data: Dict[str, Any]) -> Dict[str, Any]:
    alias_exact: Dict[Tuple[str, str], Dict[str, Any]] = {}
    alias_normalized: Dict[Tuple[str, str], Dict[str, Any]] = {}
    ...
    for alias in alias_data.get("aliases", []):
        package_scopes = alias.get("package_scopes", []) or []
        ...
        for key in keys:
            if not key:
                continue
            alias_exact[(key, "|".join(package_scopes))] = alias
            alias_normalized[(normalize_text(key), "|".join(sorted(normalize_text(p) for p in package_scopes)))] = alias
...
def _iter_alias_candidates(entry: ParsedEntry, alias_data: Dict[str, Any]):
    ...
    for alias in alias_data.get("aliases", []):
        ...
        if alias.get("prisma_title") == entry_title_exact:
        ...
        if entry_title_norm in normalized_alias_titles:
            yield alias, "alias_normalized", 0.95
