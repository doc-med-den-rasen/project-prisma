...
from .matcher_v09_3 import apply_matching
from .models_v09_3 import ParsedEntry, RuleFinding
from .rules_v09_3 import MESSAGES_V09_3, RULES_V09_3
...
def check_duplicate_same_service(entries: List[ParsedEntry], counter: Dict[str, int]) -> List[RuleFinding]:
    ...
    if not RULES_V09_3["duplicate_same_service"]["enabled"]:
        return findings
    ...
    for group in by_signature.values():
        ...
        findings.append(
            RuleFinding(
                ...
                severity=RULES_V09_3["duplicate_same_service"]["severity"],
                package=anchor.package,
                entry_id_primary=anchor.entry_id,
                entry_id_secondary=None,
                title_primary=anchor.display_title,
                title_secondary=None,
                message=MESSAGES_V09_3["duplicate_same_service"].format(title=anchor.title, count=len(group)),
...
def check_included_redundancy(entries: List[ParsedEntry], lep_rule_data: Dict[str, Any], counter: Dict[str, int]) -> List[RuleFinding]:
    findings: List[RuleFinding] = []
    if not RULES_V09_3["included_redundancy"]["enabled"]:
        return findings
        ...
        for entry in package_entries:
            ...
            findings.append(
                RuleFinding(
                    ...
                    severity=RULES_V09_3["included_redundancy"]["severity"],
                    package=entry.package,
                    entry_id_primary=entry.entry_id,
                    entry_id_secondary=None,
                    title_primary=entry.display_title,
                    title_secondary=None,
                    message=MESSAGES_V09_3["included_redundancy_grouped"].format(
                        parent=entry.title,
...
def check_excluded_but_separate_check(entries: List[ParsedEntry], lep_rule_data: Dict[str, Any], counter: Dict[str, int]) -> List[RuleFinding]:
    findings: List[RuleFinding] = []
    if not RULES_V09_3["excluded_but_separate_check"]["enabled"]:
        return findings
        ...
        for entry in package_entries:
            ...
            findings.append(
                RuleFinding(
                    ...
                    severity=RULES_V09_3["excluded_but_separate_check"]["severity"],
                    package=entry.package,
                    entry_id_primary=entry.entry_id,
                    entry_id_secondary=None,
                    title_primary=entry.display_title,
                    title_secondary=None,
                    message=MESSAGES_V09_3["excluded_but_separate_check_grouped"].format(
                        parent=entry.title,
...
def run_engine_v09_3(parser_entries: List[Dict[str, Any]], lep_rules_path: str | Path, lep_aliases_path: str | Path) -> Dict[str, Any]:
