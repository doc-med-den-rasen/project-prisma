"""
PRISMA - XLSX App für Zweizeilen-Format
=======================================

Diese App ist auf die KIS-XLSX-Dateien abgestimmt,
bei denen jeder logische Pflegeplaneintrag aus zwei Excel-Zeilen besteht.
"""
...
from prisma_xlsx_paired import parse_pflegeplan_xlsx_paired
...
def save_uploaded_file_temporarily(uploaded_file) -> Path:
    """Speichert die Upload-Datei temporär auf der Platte."""
        ...
        rows.append(
            {
                ...
                "AnlegeDat": str(entry.get("anlegedat") or ""),
                "Erf.zeit": str(entry.get("erfzeit") or ""),
...
st.markdown(
    """
Diese Fassung ist auf das **Zweizeilen-Format** deiner KIS-XLSX abgestimmt:

- **Zeile N** = Titel / Pflegeplaneintrag
- **Zeile N+1** = Status + Zyklentext + Metadaten

Genau das war der Knackpunkt in deiner Testdatei.
"""
)
...
try:
    ...
    parsed_entries, parser_stats = parse_pflegeplan_xlsx_paired(temp_path)
...
st.divider()
st.success("PRISMA Zweizeilen-Parser geladen.")
