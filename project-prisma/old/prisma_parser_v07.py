...
Fokus von v07:
- robuster XLSX-Import für das KIS-Zweizeilen-Format
- strengere Paket-Erkennung
- Eigenschaften/Details bleiben unter dem passenden Kontext
- pro Paket wird standardmäßig die *letzte aktive Version* verwendet
  (wichtig gegen doppelte PMD-Pakete über den gesamten Aufenthalt)
"""
...
# Im Zweizeilen-Format steckt der Zyklentext praktisch in Spalte 3
# der Status-Zeile, obwohl die sichtbare Kopfzeile "Zyklentext" anders
# aussehen kann. Diese Zuordnung basiert auf den realen Testdateien.
PAIR_COL_TITLE = 1
PAIR_COL_STATUS = 1
PAIR_COL_ZYKLUS = 3
...
def normalize_title_key(title: str) -> str:
    """Normalisiert einen Titel für Vergleiche / Deduplizierung."""
    return clean_text(title).lower()


def package_sort_key(package_name: str) -> tuple:
    """
    Sortiert PMD-Pakete mit führender Nummer zuerst in natürlicher Reihenfolge.
    Manuelle Pakete folgen danach alphabetisch.
    """
...
def detect_header_row(ws, scan_rows: int = 15) -> int:
    """
    Sucht nach der Kopfzeile mit 'Bezeichnung Pflegeplaneintrag'.
    Fallback = 1.
    """
...
def is_package_title(title: str) -> bool:
    """
    Erkennt Paket-/Bereichszeilen bewusst streng.

    Paket ist:
    - PMD-Titel mit '(PMD)'
    - bekannte manuelle Pakete (Aufnahme, Tagesroutine, ...)
    - bekannte Serienpakete wie 'Infusionen 15/18/25'
    """
    t = clean_text(title)
    tl = t.lower()
...
def extract_logical_pairs(ws) -> tuple[list[dict[str, Any]], int]:
    """
    Liest das KIS-Zweizeilen-Format ein:
    ...
    while row_idx <= ws.max_row:
        ...
        # Unerwartete Kopfzeilen mitten in der Datei ignorieren
...
def parse_pflegeplan_xlsx_v07(xlsx_path: str | Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """
    Parst die Pflegeplanung und liefert:
    1. flache Eintragsliste für die App
    2. Parser-Statistik
    """
    ...
    # --------------------------------------------------------
    # 1) AKTIVE PAKET-OCCURRENCES EINLESEN
    # --------------------------------------------------------
    ...
    for pair in active_pairs:
        ...
        if current_package is None:
            # Sicherheitsnetz für Datenfragmente ohne vorheriges Paket
            current_package = {
                "package": "(ohne Paket)",
        ...
        if zyklus or is_action_title(title):
            ...
            # Details/Eigenschaften hängen standardmäßig an der letzten Leistung.
            # Falls noch keine Leistung existiert, bleiben sie auf Paketebene.
    ...
    # --------------------------------------------------------
    # 2) PRO PAKET NUR DIE LETZTE AKTIVE VERSION BEHALTEN
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 3) FLACHE EINTRÄGE FÜR DIE APP ERZEUGEN
    # --------------------------------------------------------
    ...
    stats: dict[str, Any] = {
        ...
        "format_note": "Titel in Zeile N, Status/Zyklus in Zeile N+1; pro Paket wird die letzte aktive Version verwendet.",
    }
...
if __name__ == "__main__":
    print("Dies ist der Parser v07 von PRISMA.")
    print("Bitte starte die Streamlit-Oberfläche: prisma_app_v07.py")
